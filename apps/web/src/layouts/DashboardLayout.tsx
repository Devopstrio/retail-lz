...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Store Overview', path: '/', icon: LayoutDashboard },
    { name: 'Order Management', path: '/orders', icon: ShoppingCart },
    { name: 'Inventory Hub', path: '/inventory', icon: Package },
    { name: 'Customer Insights', path: '/customers', icon: Users },
    { name: 'Retail Analytics', path: '/analytics', icon: BarChart3 },
    { name: 'Supply Chain', path: '/supply-chain', icon: Truck },
    { name: 'FinOps & Costs', path: '/costs', icon: DollarSign },
  ];
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-blue-400 bg-clip-text text-transparent">RETAIL-LZ</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Retail Ops Lead</p>
                <p className="text-xs text-slate-500 truncate">Store Engineering</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search orders, products, or store performance..." 
